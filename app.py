...
CHECK_INTERVAL = 30  # Intervalo de verificación de proxies en segundos (30 segundos)
...
def fetch_proxies():
    """Devuelve el proxy preconfigurado de Geonode."""
    proxy = f"http://{GEONODE_USERNAME}:{GEONODE_PASSWORD}@{GEONODE_DNS}"
    logging.info(f"🔍 Usando el proxy: {proxy}")
    return [proxy]

def check_stock(proxy):
    """Verifica el stock de un producto usando un proxy válido."""
    headers = {
...
def main():
    send_telegram_notification("✅ El script está operativo y verificando proxies cada 30 segundos.")

    last_out_of_stock_notification_time = 0

    while True:
        # Cargar proxy desde Geonode cada 30 segundos
        proxies = fetch_proxies()
        if not proxies:
            send_telegram_notification("❌ No se pudieron cargar proxies. Intentando nuevamente en 30 segundos...")
        ...
        if not working_proxies:
            send_telegram_notification("❌ No se encontraron proxies funcionales. Intentando nuevamente en 30 segundos...")
            time.sleep(CHECK_INTERVAL)
            continue

        send_telegram_notification(f"✅ Proxies funcionales encontrados: {len(working_proxies)}")

        # Verificar stock solo cuando hay proxies funcionales
        for proxy in working_proxies:
            if check_stock(proxy):
                logging.info("¡Producto disponible!")
                last_out_of_stock_notification_time = 0
                break  # Salir del ciclo si el producto está disponible
        ...
        time.sleep(CHECK_INTERVAL)  # Esperar antes de intentar nuevamente
